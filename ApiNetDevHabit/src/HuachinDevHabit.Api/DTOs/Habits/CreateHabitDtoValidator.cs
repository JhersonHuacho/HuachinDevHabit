	...
	public sealed class CreateHabitDtoValidator : AbstractValidator<CreateHabitDto>
	{
		...
		public CreateHabitDtoValidator()
		{
			...
			RuleFor(x => x.Description)
				...
				.WithMessage("Description cannot exceed 500 characteres.");

			RuleFor(x => x.Type)
				.IsInEnum()
				.WithMessage("Type is required and must be a valid enum value.");
		...
		private static bool IsTargetUnitCompatibleWithType(HabitType type, string unit)
		{
			...
			return type switch 
			{
				...
				// Mesurable habits can use any of the alllowed units
