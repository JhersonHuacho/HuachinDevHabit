...
namespace HuachinDevHabit.Api.Controllers
{
	...
	public class EntriesController : ControllerBase
	{
		...
		public EntriesController(ApplicationDbContext applicationDbContext, UserContext userContext, LinkService linkService)
		{
			...
			DataShapingService dataShapingService)
		{
			...
			EntryDto? entry = await _applicationDbContext.Entries
			...
			if (query.IncludeLinks)
			{
				((IDictionary<string, object?>)shapedEntryDto)[nameof(ILinkResponse.Links)] =
			...
			IValidator<CreateEntryBatchDto> validator)
		{
			...
			List<Habit> existingHabits = await _applicationDbContext.Habits
			...
			_applicationDbContext.Entries.AddRange(entries);
			await _applicationDbContext.SaveChangesAsync();
			...
			IValidator<UpdateEntryDto> validator)
		{
			...
			Entry? entry = await _applicationDbContext.Entries
		...
		public async Task<ActionResult> DeleteEntry(string id)
		{
			...
			_applicationDbContext.Entries.Remove(entry);
			await _applicationDbContext.SaveChangesAsync();
		...
		public async Task<ActionResult<EntryStatsDto>> GetStats()
		{
			...
			var entries = await _applicationDbContext.Entries
				.Where(e => e.UserId == userId)
				.OrderBy(e => e.Date)
				.Select(e => new { e.Date })
				.ToListAsync();
			...
			bool hasPreviousPage)
		{
			List<LinkDto> links =
			[
				_linkService.Create(nameof(GetEntries), "self", HttpMethods.Get, new
			{
			...
			if (hasNextPage)
			{
				links.Add(_linkService.Create(nameof(GetEntries), "next-page", HttpMethods.Get, new
				{
			...
			if (hasPreviousPage)
			{
				links.Add(_linkService.Create(nameof(GetEntries), "previous-page", HttpMethods.Get, new
				{
					page = parameters.Page - 1,
					pageSize = parameters.PageSize,
					fields = parameters.Fields,
		...
		private List<LinkDto> CreateLinksForEntry(string id, string? fields, bool isArchived)
		{
			List<LinkDto> links =
			[
				_linkService.Create(nameof(GetEntry), "self", HttpMethods.Get, new { id, fields }),
				_linkService.Create(nameof(UpdateEntry), "update", HttpMethods.Put, new { id }),
				isArchived ?
					_linkService.Create(nameof(UnArchiveEntry), "un-archive", HttpMethods.Put, new { id }) :
					_linkService.Create(nameof(ArchiveEntry), "archive", HttpMethods.Put, new { id }),
				_linkService.Create(nameof(DeleteEntry), "delete", HttpMethods.Delete, new { id })
			];
